...
- Models: `Meeting`, `Transcript`, `TranscriptSegment`, `TodoItem`, `AppSettings`, `MeetingStatus`  
- `WorkingDirectoryStore`: create layout, read/write meta, index meetings  
- Schema version + migration stub  
- No UI beyond what’s needed for debug if any  

**Manual test:** Unit tests create temp dir, write meeting, reload index.

**Tests:** 100% of Domain + Storage pure logic (create, load, migrate, corrupt-file errors).

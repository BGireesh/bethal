...
- `TranscriptionEngine` protocol + `AppleSpeechTranscriptionEngine` (on-device preferred) + mock  
- `TranscriptionCoordinator` → `transcript.json` + meeting status `transcribed` / `failed`  
- Meetings list Transcribe / Re-transcribe / Retry + progress sheet  
- Notes: `docs/transcription-notes.md`  

**Manual test:** Record short speech; grant Speech Recognition; Transcribe → status Transcribed; `transcript.json` on disk.

**Tests:** Audio resolver, coordinator orchestration with mock engine, ViewModel retry (100% Domain/Services; Apple Speech wrapper excluded).
